...
- **Cards can repeat.** Give a card a due date and pick how often it comes back — every day, week, two weeks, month or year — under **Repeat** in the due-date menu; its tile shows ⟳ beside the date. Marking it done keeps it as the record of that time and puts the next one on the board: at the bottom of the area the series was set up in (even if the card was moved along to "Done" first), due at the next date in the series after today, with its checklist unticked and its labels, assignee and reminders carried over. Done late, the dates that have gone by are skipped rather than piled up as overdue cards. A card due on the 31st lands on the last day of shorter months and returns to the 31st, and a card due at nine stays at nine when the clocks change. The series moves on to the new card, so ticking the old one off and on again cannot create a second. It works the same through the REST API (`repeatEvery` on `PUT /api/data/card`, which answers with the new card as `next`) and through MCP (`repeat` on `createCard` and `updateCard`), and a duplicated card repeats like its original. The card's history records setting it up, stopping it, and each card it put on the board.
- **A card can be on several people.** Not every job is one person's. The **Assignee** menu in a card now puts each person on or takes them off with one click and stays open for the next; **Unassigned** at the top takes everybody off. The tile, the search results and the card itself show everyone on it as overlapping avatars, with all the names on hover. The board filter and **My work** find a shared card for each of its people, due-date reminders go to all of them, and people put on a card by somebody else are told — nobody is told about a card they took on themselves. The card's history says who came on and who went off, and reads "took this on" when somebody put themselves on it. Upgrading carries every existing assignment over, and the old single-assignee column is removed so nothing is left to disagree with it.

  For integrations: cards now carry `assignees` (everyone, in the order they were added). `assignee`, `assigneeName`, `assigneeImage` and `assigneeType` stay, describing the first of them. `PUT /api/data/card` takes `assignees` to set everyone; the single `assignee` still works and replaces everyone with that one person — and leaving both out now leaves the people alone, where leaving out `assignee` used to take the card off whoever had it. Through MCP, cards carry `assigneeIds` alongside `assigneeId`, and `createCard` and `updateCard` take `assigneeIds`. `claimCard` still only succeeds on a card nobody is on, and stays atomic: two agents claiming one free card at the same moment cannot both get it. `releaseCard` takes only the caller off.
- **A board, and My work, as a spreadsheet.** **⋮ › Export as spreadsheet** on a board downloads its cards as a CSV file — one row per card, in the board's order, with its area, title, status, due date and how it repeats, the people on it, labels, checklist progress, comment and attachment counts, the description and a link back to the card. On the dashboard's My work view the same menu entry downloads that list, with the board each card is on. The file opens with a double-click: the headings are in the instance's language, and so is the separator — a comma in English, a semicolon in the other nine languages, which is what Excel splits on there — with a byte-order mark so umlauts arrive intact and checklist progress written as "1 of 3", which no spreadsheet mistakes for a date. A cell that starts like a formula is kept as text, so opening the file never runs anything. For scripts, `format=csv` on `GET /api/data/board-export` and `GET /api/data/my-work`.
- **Boards from Wekan and Nextcloud Deck.** The dashboard's **⋮ › Import from Trello** is now **⋮ › Import boards**, and beside the Trello link it takes the export file Wekan or Nextcloud Deck writes — which of the two wrote it is read from the file, so choosing it is all there is to do. The boards come across private and yours: lists as areas, cards in order with their Markdown descriptions, checklists, labels, due dates and done state, and comments under the name of whoever wrote them. From Wekan (the board menu's *Export board → JSON*) the attached files come too, since the export carries them; a card is done when its due date was marked complete, and swimlanes fold into their lists. From Nextcloud Deck (`occ deck:export <user>`, run by an administrator — Deck has no export button) every board in the file becomes a board of its own; Deck leaves attachments out of that file, so there are none to bring. People are not carried over, and archived lists and cards stay behind, as with Trello. Both importers are tested against real exports — a Wekan of September 2026 and Nextcloud Deck 1.19, each run in a container, a board built through the tool's own API and exported the way a user would — which are kept in the repository so every change is checked against what those tools actually write. For scripts: `POST /api/data/import/file` with the file as the body.
- **Private Trello boards, from their export file.** The Trello import only ever reached public boards, because it reads the board from its public link. A private board can now come over too: in Trello, the board menu's *Print, export and share → Export as JSON*, then **Choose export file** in the same import dialog that takes Wekan and Deck files — the file is recognised as Trello's. It is the same JSON the link import reads, so it comes across the same way. The one difference is the attachments: Trello's export links to them, and for a private board Trello hands them only to somebody signed in. The person importing is, so the dialog asks them to lend the import that for a moment — **Allow in Trello** opens Trello's own page, which grants LokalBoards read access for one hour; paste the token it shows, choose **Import with the files**, and the files are downloaded as them. The token travels with that one request, is used for those downloads and kept nowhere, and never appears in a log. The step is only there for a private board that has files, on an instance with a Trello API key (`NUXT_PUBLIC_TRELLO_API_KEY`; the boards guide says how to get one in a few minutes); **Import without the files**, or an instance without a key, leaves each card listing its files as links, to fetch by hand while the board is still in Trello.
- **Pictures in a Trello card's text come along.** A picture pasted into a Trello description or comment is a link to Trello, which on a private board shows as a broken image for anybody not signed in there. On import every one of them is downloaded, stored the way a picture pasted into a LokalBoards card is, and the text pointed at the copy — from the link import as well as from a file. The dialog now says which way suits which board: **From a Trello link** for a public one, **From an export file** for everything else.
- **The Trello import brings labels and due dates, and never drops a file without a word.** Both were in Trello's JSON all along and were left out; they now come across, a label that is only a colour under its colour's name. A file that cannot be downloaded — refused by Trello, larger than 10 MB, or past the 300 a board may bring — used to vanish; it is now listed on its card as a link. Files are also fetched six at a time instead of one after another, so a board with a hundred pictures no longer keeps the request open for minutes — long enough for a reverse proxy in front of the app to give up and show an error for an import that was still going fine behind it. When a file is somebody's upload rather than Trello's answer, the server only ever fetches Trello's own attachment addresses from it, never whatever else the file names.
- **The done button in a card has a name.** Screen readers announced it as an unlabelled button; it is now "Done", and says whether it is pressed.

### Fixes

- **Deleting a card through MCP removes all of it.** The `deleteCard` tool removed the card, its comments, its attachment rows and its notifications, and left its labels, reminders, history and uploaded files behind. It now removes a card exactly the way the app does when a card is deleted for good.
- **Deleting a user takes them off every card.** An account that no longer existed stayed on the cards it had been given, as a face on the tile belonging to nobody.

### Security

- **Only a board's members can be put on its cards.** `PUT /api/data/card` accepted any account id as the assignee, and that account was then sent a notification naming the card — a card title on a private board, delivered to somebody with no access to the board. Anyone who is neither the board's owner nor invited to it is now left off, the same way labels from another board are. The MCP tools already refused them.

### Internal

- **The dev server no longer migrates a database that is not on this machine.** `npm run dev` with a `.env` pointing at a live database applied every migration it found to that database — including ones the live instance's older release could not read, whose boards then stopped loading. On the dev server a remote database is now left as it is, with a warning in the console; `NUXT_DEV_MIGRATE=true` migrates it anyway. A local database migrates as before, and a built instance is unaffected.
- **Signing in to the dev server works in Safari again.** Since v0.38.2 the session cookie is `Secure` whenever `NUXT_BOARDS_URL` is https — right for a deployment, wrong for `npm run dev`, whose `.env` names the live address while the dev server itself is reached over plain `http://localhost`. Safari refuses a Secure cookie there, so signing in answered as though it had worked and the sign-in page came straight back, `get-session` answering 401. On the dev server only the request itself decides now; a built instance is unaffected.
- **Two browser tests that had stopped testing anything are back.** `tests/cleanup` checks that deleting a card, an area or a board leaves nothing behind, and had been failing since v0.35 made deleting archive: it now deletes for good, the way the archive does, and checks the new assignee rows too. `tests/code-copy` counted the copy buttons the moment a code block appeared, before the page had added them; it now waits for them, and passes every time rather than most of the time.

### Documentation

- **The MCP guide no longer says OAuth is missing.** Its section for other clients still ended on "if it insists on OAuth, it cannot — that is the one gap, and it is on the list", three sections below the guide to connecting with OAuth that shipped in v0.37.0. It now says such a client can use OAuth too, and links there. The guide's opening line, the start page of the documentation, the README and `AGENTS.md` described MCP as working through an API key only; each now mentions the OAuth route as well, and where to manage those connections (**Settings → Connected apps**).
